...
struct NewMoodView: View {
    ...
    var body: some View {
        VStack{
            Text("write about your mood in few lines 🙂").padding(.top, (UIApplication.shared.windows.first?.safeAreaInsets.top)!+UIScreen.main.bounds.height/6).foregroundColor(.white)
            ...
            if text != ""{
            Button {
                model.moods.append(["date": Date(), "content": text])
                model.moodsCompleted = true
                model.sendData()
                model.getData()
