...
struct AllAchievementsView: View {
    ...
    func dateFormatter(i: Int) -> String{
        let date = model.achievements[i]["date"] as! Timestamp
        ...
        VStack{
            ScrollView{
                LazyVGrid(columns: columns) {
                    ForEach(0..<model.achievements.count, id: \.self){i in
                    VStack{
                        HStack{
                        ...
                        Spacer()
                        Text(model.achievements[i]["content"] as! String).font(.system(size: 12))
                        Spacer()
                        HStack{
